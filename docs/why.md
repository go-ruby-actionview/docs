...
`go-ruby-actionview/actionview` reimplements Ruby's `actionview` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-actionview/actionview` directly, with no Ruby runtime;
...
Because `actionview` is pure Go with cgo disabled, it also compiles to **WebAssembly**
